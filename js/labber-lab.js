...
// DB·신규 시스템 없음, 정적 데이터. 카드마다 반복되는 "획득처 : null" /
// "DESIGN BY 냐수" 문자열을 HTML에 박지 않고 이 데이터 + renderTraits()로 렌더링.
//
// acquisition: { label, url }
//   - label && url  → 획득처 이름을 <a>로 렌더링 (클릭 시 url 이동)
//   - label만 있음   → 일반 텍스트로 렌더링 (클릭 불가)
//   - label 없음(null) → "null" 텍스트 렌더링 (클릭 불가, href="#" 가짜 링크 금지)
//   추후 획득처가 정해지면 label/url 값만 채우면 자동으로 링크가 활성화됨.
// artwork: 파일명(확장자 제외). 실제 이미지 준비 전이므로 항상 주석 처리된 <img>로 렌더링.
//   준비되면 이 위치에 이미지 파일만 올리고 아래 두 줄을 교체하면 됨
//   (renderTraitCard의 artwork 주석 처리 로직 참고).
...
//   enabled:false → <a>(href 없음, 이동/# 튐 없음, aria-disabled) + data-href 로 목적지 보관.
//   공개 시 이 객체의 enabled 를 true 로만 바꾸면 실제 <a href> 로 렌더된다(구조 변경 불필요).
...
const TRAIT_DATA = {
  // image: 특성 "설명 이미지" (LABBER 관리소 전용). 상점 모듈 아이템 이미지와는 별개.
  //        없으면 ARTWORK placeholder 유지. 파일: images/labber/trait_pod_*.png
  // anchor: 특성 항목의 안정적 DOM id. LABBER 상점 상품 설명의 "적용 특성" 링크
  //   (labber-lab.html?tab=traits#<anchor>) 목적지. renderTraitCard 가 <article id> 로 렌더.
  pod: [
    { name: '원형 포드', grade: 'standard', artwork: 'pod-circle', anchor: 'trait-pod-round',
      image: '../images/labber/trait_pod_round.png',
      desc: '둥근 구형의 포드입니다.',
      acquisition: LL_ACQ_SHOP, designer: LL_TRAIT_DESIGNER },
    { name: '원통형 포드', grade: 'standard', artwork: 'pod-cylinder', anchor: 'trait-pod-cylinder',
      image: '../images/labber/trait_pod_cylinder.png',
      desc: '원기둥 형태의 포드입니다.<br>물병이나 캡슐처럼 둥근 면을 가진 원통형입니다.',
      acquisition: LL_ACQ_SHOP, designer: LL_TRAIT_DESIGNER },
    { name: '삼각형 포드', grade: 'special', artwork: 'pod-triangle', anchor: 'trait-pod-triangle',
    ...
    { name: '사각형 포드', grade: 'special', artwork: 'pod-square', anchor: 'trait-pod-square',
      image: '../images/labber/trait_pod_square.png',
      desc: '각이 있는 형태의 포드입니다.<br>정면을 기준으로 네 변이 드러나야 합니다.',
      acquisition: LL_ACQ_CRAFT, designer: LL_TRAIT_DESIGNER },
    { name: '반원형 포드', grade: 'special', artwork: 'pod-semicircle', anchor: 'trait-pod-semicircle',
  ...
  cartridge: [
    { name: '액체 돌출', grade: 'standard', artwork: 'cartridge-protrude', anchor: 'trait-liquid-protrusion',
      image: '../images/labber/trait_cartridge_liquid_protrude.png',
      desc: '포드 내부의 액체가 포드 바깥으로 돌출되어<br>자유로운 형태를 이룰 수 있습니다.',
      acquisition: LL_ACQ_SHOP, designer: LL_TRAIT_DESIGNER },
    { name: '액체 부착', grade: 'standard', artwork: 'cartridge-attach', anchor: 'trait-liquid-attach',
      image: '../images/labber/trait_cartridge_liquid_attach.png',
      desc: '포드의 액체 일부가 래버의 신체나 의상 등에<br>묻어 있는 형태로 표현될 수 있습니다.',
      acquisition: LL_ACQ_SHOP, designer: LL_TRAIT_DESIGNER },
    { name: '포드 분할', grade: 'special', artwork: 'cartridge-split', anchor: 'trait-pod-split',
      image: '../images/labber/trait_cartridge_pod_split.png',
      desc: '포드를 2개의 독립된 공간으로 나눌 수 있습니다.<br>분할된 포드에는 모두 래버의 액체가 담겨 있어야 합니다.',
      acquisition: LL_ACQ_CRAFT, designer: LL_TRAIT_DESIGNER },
    { name: '포드 선형 연장', grade: 'special', artwork: 'cartridge-pod-linear-extension', anchor: 'trait-cartridge-pod-linear-extension',
      image: '../images/labber/trait_cartridge_pod_linear_extension.png',
      desc: '포드의 일부가 가늘고 긴 선형으로 연장되어<br>자유롭게 휘어진 형태를 이룰 수 있습니다.',
      acquisition: LL_ACQ_CRAFT, designer: LL_TRAIT_DESIGNER },
  ],
  // 서브젝트는 그룹(어류/조류/파충류/특이)으로 묶어 렌더링
  subject: [
    { group: '어류', items: [
      { name: '흰동가리', grade: 'standard', artwork: 'subject-clownfish',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '개복치', grade: 'standard', artwork: 'subject-sunfish',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '베타', grade: 'standard', artwork: 'subject-betta',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
    ] },
    { group: '조류', items: [
      { name: '참새', grade: 'standard', artwork: 'subject-sparrow',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '까마귀', grade: 'standard', artwork: 'subject-crow',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '오리', grade: 'standard', artwork: 'subject-duck',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
    ] },
    { group: '파충류', items: [
      { name: '볼파이톤', grade: 'standard', artwork: 'subject-ballpython',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '크레스티드 게코', grade: 'standard', artwork: 'subject-crestedgecko',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '쿠터 거북이', grade: 'standard', artwork: 'subject-cooterturtle',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
    ] },
    { group: '특이', items: [
      { name: '크리쳐', grade: 'special', artwork: 'subject-creature',
        desc: '직접 디자인한 크리쳐를 서브젝트로 사용할 수 있습니다.<br>오너캐 등의 개인 창작 캐릭터를 사용할 수 있으나,<br>종족에 소속된 개체는 사용할 수 없습니다.',
        acquisition: { label: null, url: null }, designer: LL_TRAIT_DESIGNER },
      { name: '종족', grade: 'restricted', artwork: 'subject-species',

...
-- bulk_link_tails_owner.sql을 그대로 복제해 species_name만 '융용'으로 바꾼 버전.
--
-- 목적: 관리자 페이지(pages/yungyong-bulk-transfer.html)에서 뽀가 직접 입력한
--       (사이트 유저, 융용 개체번호) 조합만 처리한다.
...
--   species_name = '융용' AND owner_user_id IS NULL AND owner_is_offsite = true
--   → 뽀가 지정한 융용 중 "현재 오프사이트 상태인 개체"만 연결 대상이 된다.
--
-- 권한: auth.users.raw_app_meta_data->>'role' = 'admin' 인 계정만 실행 가능.
--       (bulk_link_tails_owner와 동일한 사유로 app_metadata.role을 사용한다.)
-- ============================================================

CREATE OR REPLACE FUNCTION bulk_link_yungyong_owner(p_items jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    IF NOT FOUND THEN
      v_skipped := v_skipped || jsonb_build_object(
        'char_id', v_char_id, 'char_number', v_char_number,
        'reason', '조건 불일치로 건너뜀 (융용/오프사이트/소유자 미지정 조건 중 하나가 검증 이후 바뀜)'
      );
      CONTINUE;
    END IF;
    ...
    SELECT id, name, '융용', NULL, v_old_owner_nick, v_new_owner_id, v_new_owner_nick, '일괄 소유권 연결'
  ...
  SELECT
    ...
    'bulk_yungyong_owner_link', 'character', NULL, '융용 소유권 일괄 연결',
...
REVOKE ALL ON FUNCTION public.bulk_link_yungyong_owner(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.bulk_link_yungyong_owner(jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.bulk_link_yungyong_owner(jsonb) TO authenticated;
